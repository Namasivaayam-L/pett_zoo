...
class DQN:
    def __init__(self, num_actions, num_states, width, num_layers,batch_size, gamma, learning_rate, model_path, log_file,fine_tune_model_path=None):
    ...
    def act(self, state, epsilon):
        if np.random.rand() <= epsilon:
            ...
            print(f'Random q_value:{q_values}',file=self.log_file)
        else:
            q_values = np.argmax(self.model.predict(state[np.newaxis, ...]))
            print(f'Predicted q_value:{q_values}',file=self.log_file)
        return q_values
    ...
    def learn(self, experience):
        ...
        with tf.GradientTape() as tape:
            ...
            print(f'loss: {loss}',file=self.log_file)
