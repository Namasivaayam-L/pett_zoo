...
class DQN:
    ...
    def act(self, state, epsilon):
        if np.random.rand() <= epsilon:
            ...
            q_values = np.argmax(self.model.predict(state[np.newaxis, ...],verbose=0))
        return q_values
    
    def learn(self, experience):
        # Reshape individual observations into batches
        # curr_state, action, reward, next_state = tuple(map(lambda x: tf.convert_to_tensor(x,dtype=tf.float32),experience))
        curr_state, action, reward, next_state = experience
        curr_state, next_state = tuple(map(lambda x: np.reshape(x,[self.batch_size,self.num_states]), (curr_state, next_state)))
        
        q_values = self.model.predict(curr_state,verbose=0)
        q_target = reward + self.gamma * np.max(self.model.predict(next_state,verbose=0))

        # Update target Q-values in the Q-values array
        q_target_vec = q_values.copy()
        q_target_vec[0][action] = q_target
        # print('before updating wts')
        # Calculate loss and apply gradients
        with tf.GradientTape() as tape:
            q_predictions = self.model(curr_state)
